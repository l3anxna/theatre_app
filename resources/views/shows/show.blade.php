<x-app-layout>
    <div class="mx-auto max-w-6xl space-y-10">
        ...
        <section class="grid gap-7 lg:grid-cols-[minmax(220px,300px)_1fr]">
            <div class="aspect-[2/3] overflow-hidden rounded-2xl bg-gradient-to-br from-[#F1D8D3] via-[#F1D8D3] to-[#EEE6DB] shadow-2xl shadow-black/30">
                @if ($show->poster_image)
                    <img src="{{ $show->poster_image }}" alt="Poster for {{ $show->title }}" class="h-full w-full object-cover">
                @else
                    <div class="flex h-full flex-col justify-end p-7"><span class="mb-auto text-xs font-semibold uppercase tracking-[0.2em] text-[#B7791F]">Stagebook</span><span class="text-3xl font-bold leading-tight">{{ $show->title }}</span></div>
                @endif
            </div>
            <div class="py-2">
                <p class="text-sm font-semibold uppercase tracking-[0.18em] text-[#B7791F]">{{ $show->genre ?? 'Theatre production' }}</p>
                <h1 class="mt-2 text-4xl font-bold tracking-tight text-[#2D2926] sm:text-5xl">{{ $show->title }}</h1>
                <p class="mt-4 text-lg leading-8 text-[#554E47]">{{ $show->description }}</p>
                <div class="mt-6 flex flex-wrap items-center gap-x-5 gap-y-3 text-sm">
                    <p class="font-semibold text-[#B7791F]" aria-label="Rated {{ number_format($show->reviews_avg_rating ?? 0, 1) }} out of 5 from {{ $show->reviews_count }} reviews">★ {{ number_format($show->reviews_avg_rating ?? 0, 1) }}/5 <span class="font-normal text-[#746D64]">{{ $show->reviews_count }} reviews</span></p>
                    <p class="text-[#554E47]">⌖ {{ $show->venue?->name ?? 'Venue to be announced' }}</p>
        ...
        <section class="grid gap-6 lg:grid-cols-2">
            <div class="rounded-2xl border border-[#D8CEC1] bg-[#FFFCF7] p-6"><h2 class="text-xl font-bold">Venue & run</h2><p class="mt-3 text-[#554E47]">{{ $show->venue?->name ?? 'Venue to be announced' }}</p><p class="mt-2 text-[#746D64]">Performance times and ticket links will appear here as they are announced.</p></div>
            <div class="rounded-2xl border border-[#D8CEC1] bg-[#FFFCF7] p-6"><h2 class="text-xl font-bold">Cast</h2><ul class="mt-4 grid gap-3 sm:grid-cols-2">@forelse ($show->actors as $actor)<li><a class="font-medium text-[#2D2926] hover:text-[#B7791F]" href="{{ route('actors.show', $actor->slug) }}">{{ $actor->name }}</a>@if ($actor->pivot->character_name)<p class="text-sm text-[#746D64]">{{ $actor->pivot->character_name }}</p>@endif</li>@empty<li class="text-[#746D64]">Cast to be announced.</li>@endforelse</ul></div>
        </section>

        <section id="review" class="border-t border-[#D8CEC1] pt-8"><h2 class="text-2xl font-bold text-[#2D2926]">Community reviews</h2>
            @auth
